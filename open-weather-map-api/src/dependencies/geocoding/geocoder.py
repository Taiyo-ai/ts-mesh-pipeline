...
class Geocoder:
    ...
    def get_locations_metadata(self, df, df_meta):
        ...
        df_meta['id'] = df['id']
        df_meta['country_name'] = df['country'].apply(
        ...
        df_meta['location'] = df['city'].apply(
            lambda x: locations[x]['address'])
        df_meta['map_coordinates'] = df['city'].apply(
            lambda x: locations[x]['coordinates'])
